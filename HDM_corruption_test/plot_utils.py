...
#        save_image(self._merge(images, [self.n_img_y, self.n_img_x]), self.DIR + "/" + name, normalize=True)
        

        test = self._merge(images, [self.n_img_y, self.n_img_x])
        #print("test1 : ",test.shape) #280x280 image

        #test = test.reshape(280,280,1)


        #save_image(test.data, self.DIR + "/" + name, normalize=True) #######################
        #save_image(test.data, self.DIR + "/" + name) 
        #save_image(test.data, "./imgs/" + name, normalize=True)
        #save_image(gen_imgs.data, "images/%d.png" % batches_done, nrow=n_row, normalize=True)
        plt.imsave(self.DIR + "/" + name, test.data, cmap = plt.cm.gray)
    ...
    def _merge(self, images, size):
        ...
        for idx, image in enumerate(images):
            ...
            image_  = resize(image, (w_, h_), order=3)
...
#        save_image(self._merge(images, [self.n_img_y, self.n_img_x]), self.DIR + "/" + name, normalize=True)
        ...
        test = self._merge(images, [self.n_img_y, self.n_img_x]).data
        plt.imsave(self.DIR + "/" + name, test.data, cmap = plt.cm.gray)
